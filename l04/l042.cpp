...
stack<Point> GrahamScan(vector<Point> &points){
    ...
    sort(points.begin(), points.end(), [minPoint](Point a, Point b){
        double side = testSide(minPoint, a, b);
        if (side < 0){
            return true;
        }
        else if (side > 0){
            ...
            double distA = distanceBetween(minPoint, a);
            double distB = distanceBetween(minPoint, b);
            return distA < distB;
        }
    });
    ...
    for (int i = 2; i < (int) points.size(); i++){
        ...
        convexHull.pop();
        while (testSide(convexHull.top(), top, points[i]) > 0 && convexHull.size() > 1){
            top = convexHull.top();
            convexHull.pop();
        }
        convexHull.push(top);
...
void part2(){
    ...
    vector<Point> points;
    copy(pointss.begin(), pointss.end(), back_inserter(points));
    stack<Point> convexHull2;
    convexHull2 = GrahamScan(points);
    vector<Point> convexHull;
    cout << convexHull2.size() << endl;
    while (!convexHull2.empty()){
        convexHull.push_back(convexHull2.top());
        convexHull2.pop();
    }
    cout << convexHull.size() << endl;
    Render render;
    ...
    //     }
    // }
    //add the last point of the convex hull to the beginning
    convexHull.insert(convexHull.begin()+0, points[0]);
    //draw the convex hull points
    for (int i = 0; i < (int) convexHull.size(); i++){
        render.coldrawcircle(convexHull[i].getXCon(), convexHull[i].getYCon(), 2,2);
        render.coldrawcircle(convexHull[i].getXCon(), convexHull[i].getYCon(), 3,2);

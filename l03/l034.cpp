...
void part4(){
    ...
    pair<unsigned long long int, unsigned long long int> p1 = pair<unsigned long long int, unsigned long long int>((points[0].getX()/(delta/2)),  (points[0].getY()/(delta/2)));
    dictionary.insert(make_pair(p1, points[0]));
    //add second point to dictionary
    pair<unsigned long long int, unsigned long long int> p2 = pair<unsigned long long int, unsigned long long int>( (points[1].getX()/(delta/2)),  (points[1].getY()/(delta/2)));
    ...
    for(int i = 2; i < (int) points.size(); i++){
        ...
        pair<unsigned long long int, unsigned long long int> p = pair<unsigned long long int, unsigned long long int>( (points[i].getX()/(delta/2)),  (points[i].getY()/(delta/2)));
        ...
        for (unsigned long long int x = xmin; x < xmin + 5; x++){
            for (unsigned long long int y = ymin; y < ymin + 5; y++){
                pair<unsigned long long int, unsigned long long int> m = pair<unsigned long long int, unsigned long long int>(x, y);
                if(dictionary.count(m) > 0){
                    temp25.push_back(dictionary.at(m));
                }
            }
        }

        bool d = false;
        int size = (int) temp25.size();
        if(size != 0 ){
            for(int j = 0; j < size; j++){
                if (points[i].distance(temp25[j]) < delta && points[i].distance(temp25[j]) != 0){
                    d= true;
                    //update delta to the distance between p and q
                    delta = points[i].distance(temp25[j]);
        ...
        if(d){
            dictionary.clear();
            for(int l = 0; l <= i; l++){
                pair<unsigned long long int, unsigned long long int> p4 = pair<unsigned long long int, unsigned long long int>( (points[l].getX()/(delta/2)),  (points[l].getY()/(delta/2)));
    ...
    cout << "Time: " << time << endl;
    fout << endl << "Randomized Algorithm: " << endl;
    fout << "The closest pair of points is: " << endl;
    r1.rtostring();
    r2.rtostring();

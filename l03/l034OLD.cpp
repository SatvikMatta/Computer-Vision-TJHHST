...
string inputFile = "points100k.txt";
...
public:
    template <class T1, class T2>
        ...
        {
            int hash1 = hash<T1>()(p.first);
            int hash2 = hash<T2>()(p.second);
            if (hash1 == hash2){
                return hash1;
            }
            else{
...
void part4(){
    ...
    //initialize delta to to the distance between the first two points
    double delta = points[0].distance(points[1]);
    //Invoke MakeDictionary for storing subsquares of side length delta/2
    unordered_map<pair<unsigned long long int, unsigned long long int>, Point, myHashFunction> dictionary;
    //add first point to dictionary
    pair<unsigned long long int, unsigned long long int> p1 = make_pair((unsigned long long int) (points[0].getX()/(delta/2)), (unsigned long long int) (points[0].getY()/(delta/2)));
    dictionary.insert(make_pair(p1, points[0]));
    //add second point to dictionary
    pair<unsigned long long int, unsigned long long int> p2 = make_pair((unsigned long long int) (points[1].getX()/(delta/2)), (unsigned long long int) (points[1].getY()/(delta/2)));
    dictionary.insert(make_pair(p2, points[1]));
    //for each point p in the list
    for(int i = 2; i < (int) points.size(); i++){
        //find the subsquare of side length delta/2 that p belongs to
        pair<unsigned long long int, unsigned long long int> p = make_pair((unsigned long long int) (points[i].getX()/(delta/2)), (unsigned long long int) (points[i].getY()/(delta/2)));
        unsigned long long int xmin = 0;
        unsigned long long int ymin = 0;
        if (p.first < 2){
            xmin = 0;
        }
        else{
        ...
        bool d = false;
        //Look up the 25 subsquares of side length delta/2 that surround the subsquare containing p
        for (unsigned long long int x = xmin; x < xmin + 5; x++){
            for (unsigned long long int y = ymin; y < ymin + 5; y++){
                pair<unsigned long long int, unsigned long long int> p3 = make_pair(x, y);
                if (dictionary.find(p3) != dictionary.end()){
                    //if a point q is found in the dictionary
                    //if the distance between p and q is less than delta
                    if (points[i].distance(dictionary[p3]) < delta && points[i].distance(dictionary[p3]) != 0){
                        d= true;
                        //update delta to the distance between p and q
                        delta = points[i].distance(dictionary[p3]);
                        //update r1 and r2 to be p and q
                        r1 = points[i];
                        r2 = dictionary[p3];
                        //update smalldist to be the distance between r1 and r2
                        smalldist = delta;
                    }
                }
            }
        ...
        if(d){
            dictionary.clear();
            for(int l = 0; l <= i; l++){
                pair<unsigned long long int, unsigned long long int> p4 = make_pair((unsigned long long int) (points[l].getX()/(delta/2)), (unsigned long long int) (points[l].getY()/(delta/2)));
                dictionary.insert(make_pair(p4, points[l]));
            }
        }
        else{
            //add p to the dictionary
            dictionary.insert(make_pair(p, points[i]));
        }
    ...
    r1.tostring();
    r2.tostring();
    cout << "The distance between them is: " << smalldist << endl;
    cout << "Time: " << time << endl;
    fout << "Randomized Algorithm: " << endl;
    fout << "The closest pair of points is: " << endl;
    r1.rtostring();
    r2.rtostring();
    fout << "The distance between them is: " << smalldist << endl;
    fout << "Time: " << time << endl;
    fout.close();
}
